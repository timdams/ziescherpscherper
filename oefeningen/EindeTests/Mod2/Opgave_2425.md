...
Gegeven volgende flowchart voor een sauna. De gebruiker kan via een menu'tje ingeven wat hij wenst te doen. Pas de flowchart toe en hou rekening met volgende zaken:

1. De veiligheidsvoorschriften tonen gewoon de tekst "OPGELET WARM!" in rode letters.
2. De sauna start met 5 houtblokken op het vuur. Ieder houtblok zorgt voor 10 graden warmte. Telkens een blok wordt toegevoegd wordt de nieuwe warmte getoond. De sauna zal nooit voorbij de gewenste warmte gaan: de gewenste temperatuur is bereikt zodra er geen blok meer bij kan zonder die warmte te overschrijden. Is het 40 graden en wenst de gebruiker 60 graden, dan moeten er dus 2 blokken toegevoegd worden. Als de gebruiker 75 graden wenst, dan zullen er in totaal 7 blokken op het vuur moeten liggen. Je mag ervan uitgaan dat de gewenste warmte minstens 10 graden hoger ligt dan de huidige warmte, zodat er altijd minstens 1 blok bijkomt.
3. Telkens het menu wordt getoond vermindert het aantal blokken met 1. (dit wil zeggen dat de 5 startblokken ogenblikkelijk 4 worden wanneer het programma opstart en het menu toont). 
...
* Een methode C (naam: ``Combineer``) waar een geheel getal en een landcode (bv "BE") aan kan meegegeven worden. De landcode is optioneel (standaard : "BE") en is daarom de laatste parameter. De methode zal de landcode en het getal samengevoegd als een string teruggeven. Als er dus 5 en "FR" wordt meegegeven, geeft de methode FR5 terug.
...
Roep de methode ``GenereerAutoKenteken`` 5 keer aan vanuit de main en bewaar de kentekens in een array.
...
Deze methode heeft bovenaan je `Program.cs` de regels `using System.Net;` en `using System.Net.Sockets;` nodig.
...
Maak een aantal hulpmethoden aan (bepaal zelf return type en eventuele parameters), met volgende eigenschappen.
...
Werking VerhoogAdres, nieuwe adres is: 192.168.1.101
```
...
namespace Sauna
{
    ...
    {
        enum MenuKeuzes { Voorschriften = 1, Opwarmen, Stoppen }

        static void Main(string[] args)
        {
            const int BLOKWARMTE = 10;
            int aantalBlokken = 5;
            MenuKeuzes gebruikersKeuze;
            do
            {
                //Toon menu en huidige warmte
                aantalBlokken--;
                Console.WriteLine($"Welkom bij de sauna ({aantalBlokken * BLOKWARMTE} graden). Je keuze? 1=voorschriften, 2= opwarmen, 3= stoppen.");
                ...
                switch (gebruikersKeuze)
                {
                    ...
                    case MenuKeuzes.Opwarmen:
                        ...
                        int gewensteWarmte = int.Parse(Console.ReadLine());
                        do
                        {
                            aantalBlokken++;
                            Console.WriteLine($"1 blok toegevoegd. Het is nu {aantalBlokken * BLOKWARMTE} graden.");
                        } while ((aantalBlokken + 1) * BLOKWARMTE <= gewensteWarmte);
                        ToonVeiligheidsVoorschriften();
                        break;
                }
...
namespace Kentekengenerator
{
    internal class Program
    {
        ...
        static string GenLandcode()
        {
            ...
            char letter1 = (char)rng.Next((int)'A', (int)'Z' + 1);
            char letter2 = (char)rng.Next((int)'A', (int)'Z' + 1);
            return letter1.ToString() + letter2.ToString();
        }

        static string Combineer(int getal, string landcode = "BE")
        {
            return landcode + getal;
        }

        static string GenereerAutoKenteken()
        {
            int getal = GenGetal();
            string landcode = GenLandcode();
            if (landcode == "XX")
            {
                return Combineer(getal);
            }
            return Combineer(getal, landcode);
        }
    }
}
```
...
namespace IPIPIP
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("Je ipadres is: ");
            ToonIPAdres(mijnIP);
            Console.WriteLine();
            Console.Write("Het masker: ");
            ToonIPAdres(mijnIP, true);
            Console.WriteLine();
            Console.WriteLine();

            Console.Write("Werking IsLokaal: ");
            if (IsLokaal(mijnIP))
            {
            ...
            {
                Console.WriteLine("Dit is geen lokaal adres.");
            }
            ...
            Console.Write("Werking IsCorrectAdres: ");
            ...
            {
                Console.WriteLine("Dit is geen correct adres");
            }

            string[] anderAdres = { "192", "168", "1", "105" };
            Console.Write("Werking IsGelijk (vergeleken met ");
            ToonIPAdres(anderAdres);
            Console.Write("): ");
            if (IsGelijk(mijnIP, anderAdres))
            {
            ...
            string[] verhoogdAdres = VerhoogAdres(mijnIP);
            Console.Write("Werking VerhoogAdres, nieuwe adres is: ");
            ToonIPAdres(verhoogdAdres);
            Console.WriteLine();
        }

        static void ToonIPAdres(string[] ip, bool gemaskeerd = false)
        {
        ...
        {
            int eersteDeel = int.Parse(ip[0]);
            return eersteDeel == 10 || eersteDeel == 192;
        }
        ...
        static bool IsCorrectAdres(string[] ip)
        {
            bool correct = true;
            int index = 0;
            while (correct && index < ip.Length)
            {
                int deel = int.Parse(ip[index]);
                if (deel < 1 || deel > 254)
                {
                    correct = false;
                }
        ...
        {
            int laatsteDeel = int.Parse(ip[3]);
            laatsteDeel++;
            if (laatsteDeel == 255)
            {
                laatsteDeel = 1;
            }
            return new string[] { ip[0], ip[1], ip[2], laatsteDeel.ToString() };
        }
