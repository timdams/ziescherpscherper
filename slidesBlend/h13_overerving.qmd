...
In [C#]{.cs} erft een klasse van **maximaal één** parent.
...
::: {.callout-tip .fragment}
Denk aan een **vogelbekdier**: tegelijk vogel en zoogdier. Erfde het van allebei, met welke `LegEi()` of `GeefMelk()` moet de compiler dan werken? [C#]{.cs} omzeilt die patstelling.
:::
...
Vergeet je `override` (en schrijf je gewoon `public void Vlieg()`), dan krijg je een **`new`**-waarschuwing: [C#]{.cs} denkt dat je de methode wil **verbergen** (*hiding*), niet **overschrijven**.
:::
