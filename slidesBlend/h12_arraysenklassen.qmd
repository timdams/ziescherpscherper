...
## Een array is een lege parking

Denk aan een object-array als een **parkeerterrein**:

* `new Student[20]` schildert de **witte vakken** (de 20 plekken)
* maar er staat nog **geen enkele auto** (object): elke plek is `null`
* met `new Student()` parkeer je telkens een auto in een vak

::: {.callout-tip .fragment}
Daarom moet je na het aanmaken van de array nog **elk object apart** met `new` maken.
:::
...
`List<>`, `Queue<>`, `Dictionary<>` ... dat `< >` heet **generics**:

* het maakt een klasse **datatype-onafhankelijk**: dezelfde `List` werkt voor `int`, `string`, `Pokemon`, ...
* tussen de `< >` zeg je welk type erin mag: `List<int>`, `List<Pokemon>`

::: {.callout-note .fragment}
Zelf generieke klassen schrijven hoeft nu nog niet (zie de appendix). Gebruiken volstaat: het `< >` zegt gewoon "lijst van ...".
:::
...
::: {.callout-tip .fragment}
In [C#]{.cs} is dit puur **lazy syntax**: het type ligt nog steeds vast (statically typed). Anders dan in JavaScript, waar `var` een echt variabel type betekent.
:::
...
## Showcase: De Slimste Mens

Een quizspel waarin elke collectie precies de juiste rol speelt:

| Collectie | Rol in de quiz |
|---|---|
| `Queue` | de **vragenstapel** (eerst gesteld, eerst aan de beurt) |
| `Stack` | de **foute vragen** (laatste fout bovenop) |
| `Dictionary` | de **score per categorie** (key = categorie) |
| `List` | **alle** gestelde vragen |

::: {.callout-tip .fragment}
De kunst is niet "ken alle collecties", maar **kies de juiste** voor het probleem. FIFO? `Queue`. Opzoeken via een sleutel? `Dictionary`.
:::
...
* **`var`** laat de compiler het type afleiden ([C#]{.cs} blijft statically typed)
