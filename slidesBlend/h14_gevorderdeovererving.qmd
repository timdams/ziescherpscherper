...
Volgens Darwin stamt alle leven af van een gemeenschappelijke voorouder. En voor de niet-darwinisten onder ons: in den beginne schiep .NET **`System.Object`**.

Hoe je het ook bekijkt: **elke** klasse in [C#]{.cs} stamt af van `System.Object`. Het is de stamvader van alles.
...
::: {.column width="55%"}
**Alles** in [C#]{.cs} erft van **`System.Object`**: je eigen klassen, `Random`, `Console`, zelfs `int` en `bool`.
...
Bonus: met een goede `ToString()` toont ook de **debugger** meteen nuttige info als je over een object hovert, in plaats van enkel de klassenaam.
:::
...
## Kopieren: shallow vs deep

`System.Object` geeft ook `MemberwiseClone()`: een **shallow copy** (ondiepe kopie).

```csharp
public Student MaakKopie()
{
    return (Student)MemberwiseClone(); // shallow copy
}
```

* **Shallow**: value-velden worden gekopieerd, maar geneste **objecten** worden **gedeeld** (enkel de referentie gaat mee)
* **Deep**: ook de geneste objecten zelf kopieren, dat vraagt **handwerk**

::: {.callout-warning .fragment}
Pas je in de kopie een gedeeld nested object aan, dan verandert het origineel mee. Voor een echte, losse kopie heb je een **deep copy** nodig.
:::
