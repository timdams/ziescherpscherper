...
Je kan geen appelen in peren veranderen zonder magie. In [C#]{.cs} heb je drie manieren:
...
Weetje: *to parse* betekent **ontleden** (van het Latijnse *pars*, "deel"). Een parser ontleedt tekst en zet ze om in een datastructuur.
:::
...
```csharp
double grondtal = 5;
double metHand = grondtal * grondtal * grondtal; // 125, maar niet voor een variabele exponent
double metPow = Math.Pow(grondtal, 3);           // 125, duidelijk en flexibel
```

* Voor een vast **kwadraat** is `getal * getal` prima
* Voor alle andere (grotere of variabele) machten: **`Math.Pow`**
...
Stel: je behaalt **9,50** en **8,50**. Hoe worden die afgerond?

```csharp
double p1 = 9.50, p2 = 8.50;
Math.Round(p1);  // 10
Math.Round(p2);  // 8  (banker's rounding!)
Math.Round(p1, MidpointRounding.AwayFromZero);  // 10
Math.Round(p2, MidpointRounding.AwayFromZero);  // 9
```

::: {.callout-warning .fragment}
Met de standaard (banker's) wordt je 8,50 een **8**, niet een 9. Voor punten wil je dus `AwayFromZero`.
:::
...
Niet elke fout is hetzelfde. Het helpt te weten **wanneer** ze opduikt:

* **Compile-time** (syntaxfouten): de compiler betrapt ze meteen, VS onderlijnt ze al tijdens het typen
* **Run-time** (logische fouten en exceptions): pas zichtbaar terwijl het programma draait, en vaak het lastigst
* **Link-time**: bij het bouwen, als externe code gekoppeld wordt (zeldzaam, voor jou nu geen zorg)

::: {.callout-tip .fragment}
Compile-time fouten zijn de "makkelijkste": de compiler zegt letterlijk wat er mis is. Run-time bugs moet je zelf opsporen, met de debugger.
:::
...
::: {.callout-important}
**De verboden prompt**: *"Schrijf de [C#]{.cs}-oplossing voor deze opgave."* Dat is als de Tour de France-winnaar de Mont Ventoux voor je laten opfietsen: je geraakt boven, maar leert niets.
:::
...
**Python** doet dit korter: de naam van het type is meteen de omzetfunctie:
