...
* de **[C#]{.cs}-syntax** en de rol van **keywords** uitleggen
...
## [C#]{.cs} is ook maar een taal

Net als elke taal bestaat [C#]{.cs} uit:

* **grammatica**: de **[C#]{.cs}-syntax**
...
* [C#]{.cs} heeft **80 gereserveerde keywords** met een vaste, eenduidige betekenis
...
[C#]{.cs} is een levende taal. Nieuwe woorden komen erbij als **contextual keywords** (bv. `get`, `set`, `value`, `var`); de lijst van 80 echte keywords verandert niet.
:::
...
[C#]{.cs} is een **strongly typed language**: je legt van bij de start vast welk **soort data** een variabele bevat.
...
## Waar maak je variabelen aan?

Maak een variabele **zo dicht mogelijk** bij de plek waar je ze gebruikt.

* Vroeger (in c/c++) zette men alle variabelen **bovenaan**: dat raden we tegenwoordig **af**
* Dicht bij het gebruik = leesbaarder, en minder kans op fouten

::: {.callout-tip .fragment}
Hoe korter de "levensduur" van een variabele, hoe makkelijker je code te volgen is.
:::
...
`int / int` geeft een `int`: [C#]{.cs} **kapt** alles na de komma af (*truncatie*), het rondt niet af. Ook `4.9` wordt zo `4`.
:::
...
Meng je een `int` met een `double`, dan kiest [C#]{.cs} het **grootste** type (hier `double`):
...
* [C#]{.cs} = **syntax** (grammatica) + **keywords** (woordenschat); statements eindigen op `;`
