...
![Jij spreekt dat plekje aan met de identifier, de computer met het geheugenadres (`0x4234FE13EF1`).](../content/assets/1_csharpbasics/geheugenNEW.png){width=62%}
...
::: {.column width="52%"}
![Elk datatype is een doosje waar maar één soort data in past.](../content/assets/1_csharpbasics/datatypedoosjes.png)
:::
...
::: {.column width="55%"}
![Bij een `short` gaat het eerste bitje naar het teken.](../content/assets/1_csharpbasics/bitliniaal.png)
:::

::::

::: {.callout-tip .fragment}
Dubbel zoveel bits verdubbelt het bereik niet, het **kwadrateert** het: 256 waarden bij een `byte`, 65 536 bij een `short`. Bij twijfel kies je gewoon **`int`**.
:::
...
![Elk type wint op precies één van de drie criteria. Het rode balkje toont welk.](../content/assets/1_csharpbasics/kommagetallen.png){width=88%}
...
::: {.callout-tip .fragment}
Meerdere variabelen van **hetzelfde type** mogen op één lijn: `string voornaam, achternaam, adres;`
:::
...
![Een **instantievariabele** (bij een object, zie het hoofdstuk klassen) krijgt *wel* automatisch de standaardwaarde van haar type.](../content/assets/1_csharpbasics/beginwaardenNEW.png){width=80%}
...
::: {.column width="80%"}
Steven laat zijn code door A.I. schrijven. Lengte en breedte zijn allebei `5` meter:
...
Dit compileert niet eens. Het `= 5` hoort **enkel** bij `breedte`, de laatste van het rijtje. `lengte` blijft zonder waarde: *use of unassigned local variable*. Wil je allebei `5`, schrijf dan `int lengte = 5, breedte = 5;`
:::
...
Heb je de oude waarde nog nodig, zet ze dan eerst in een **tweede variabele**.
:::

::: {.column width="55%"}
![](../content/assets/1_csharpbasics/overschrijvenNEW.png)
:::

::::

::: {.callout-important .fragment}
Declareer een variabele **maar één keer**. `double kdRating = 2.1;` gevolgd door `double kdRating = 3.4;` geeft *already defined in this scope*.
:::

---

## Puzzel: wissel twee variabelen om

Je hebt twee variabelen. Hoe wissel je hun waarden om?

```csharp
int a = 3;
int b = 5;
// gewenst: a wordt 5, b wordt 3
```

::: {.callout-important .fragment}
`a = b; b = a;` werkt **niet**: na `a = b` is de oude waarde van `a` al weg.
:::

::: {.fragment}
Oplossing met een **hulpvariabele**:
...
::: {.column width="48%"}
![In `3 + 2` zijn `3` en `2` de **operanden** en is `+` de **operator**.](../content/assets/1_csharpbasics/expressieontleding.png)
:::
...
Compacter om te lezen, niet sneller. `getal++` zie je overal (zeker in lussen), dus je moet het vlot kunnen **lezen**.
...
::: {.column width="48%"}
![](../content/assets/1_csharpbasics/increment.png)
:::
...
::: {.column width="55%"}
![](../content/assets/1_csharpbasics/truncatie.png)
:::
...
::: {.column width="50%"}
![](../content/assets/1_csharpbasics/salarisval.png)
:::
...
## Solutions en projecten (vervolg)
...
Dezelfde concepten uit dit hoofdstuk, maar dan in andere talen. Je hoeft die talen niet te kennen: het gaat om het **herkennen** van het patroon, ook al ziet de syntax er anders uit.

::: {.callout-important .fragment}
Steeds vaker laat je een A.I. een eerste versie schrijven en moet je die daarna lezen, beoordelen en bijsturen. Zo'n A.I. spuwt even vlot **Python**, JavaScript of C++ uit als C#.
:::
...
![](../content/assets/posters/H2_basisconcepten.png){width=55%}
