...
> **Status derde pass** (2026-08-31). [3_advancedmethod.md](3_advancedmethod.md) is herschreven. Twee inhoudelijke fouten eruit: de definitie van overloading zei "dezelfde naam **en returntype**" (het returntype mag verschillen en speelt geen rol), en er stond een verzonnen regel dat "bij twijfel de eerste parameter bepaalt". In dat voorbeeld (``Toonverhouding``) was er nooit twijfel: telkens viel één versie af omdat ``double`` niet impliciet naar ``int`` gaat. Dat is vervangen door "Wanneer de compiler niet kan kiezen", met het écht ambigue geval (CS0121) erbij. Verder: de "Voor de gevorderden"-callout sprak over named/optionele parameters terwijl ze onder overloading staat, en step-in is een ``##`` geworden want het is geen geavanceerde methode-techniek. Alle voorbeelden kwamen bijna letterlijk uit de MS-docs (``PrintDetails``/``Gift Shop``, ``BookFile``/``optName``); die zijn vervangen door eigen voorbeelden (``ToonPunten``, ``ToonFactuur``, ``Verdubbel`` van Stagiair Steven, ``ToonTitel`` uit [0_intromethods.md](0_intromethods.md), ``ToonPrijs``). Mee aangepast: [zieverder.md](zieverder.md), [summaryposterprompt.md](summaryposterprompt.md), [../../slides/h07_methoden.qmd](../../slides/h07_methoden.qmd) en [../../slidesBlend/h07_methoden.qmd](../../slidesBlend/h07_methoden.qmd).
