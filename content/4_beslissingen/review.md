...
- `[v]` **`switch` op strings** wordt niet expliciet getoond, terwijl menustructuren met letterkeuze typische beginner-oefeningen zijn. **(sectie "Switch op tekst" toegevoegd in [2_switch.md](2_switch.md): een menu met letters, de hoofdlettergevoeligheid van strings (met twee case-labels per keuze) en een `char`-variant.)**
