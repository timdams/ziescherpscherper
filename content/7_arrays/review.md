...
- `[v]` De jagged-arrays-sectie in `5_jaggedArrays.md` was heel kort en beëindigde het hoofdstuk in een soort wegtrek-modus. Geen samenvatting, geen "wanneer wel/niet jagged". **(Opgelost door het bestand te schrappen: jagged is nu een korte slotsectie onderaan [4_ndimensionalArrays.md](4_ndimensionalArrays.md), met een expliciete "herkennen, niet schrijven"-callout die naar `[,]`, H12 en H9 verwijst. Jagged kwam nergens anders in de cursus of de oefeningen voor.)**

- `[v]` In [4_ndimensionalArrays.md](4_ndimensionalArrays.md) stond bij de `boeken`-array *"We maken letterlijk een array van arrays"*. Dat is fout voor een `[,]`-array (dat is net de definitie van een jagged array) en het sprak de correcte uitleg verderop tegen, waar staat dat een array in het geheugen één 1D-blok is. Dezelfde zin sprak ook van *"een 3 bij 4 array"* terwijl `GetLength` daaronder 4 rijen en 3 kolommen telde. **(zin herschreven; het `boeken`-voorbeeld is verhuisd naar de nieuwe callout "Een rooster of een klasse?".)**
- `[v]` De jagged-sectie noemde `Length` een *methode*. Het is een eigenschap. **(rechtgezet in de nieuwe slotsectie.)**
- `[v]` De **geneste `for`-loop over een 2D-array** ontbrak volledig, terwijl practicum B, MazeGame, Conway en het examen Mod4 er niet zonder kunnen. Ook een 2D-array als methode-parameter (`static void Toon(int[,] matrix)`) kwam nergens voor. **(beide toegevoegd als eigen sectie, met `GetLength()` in de loopcondities en een waarschuwing over het verwisselen van rij en kolom.)**
...
- `[v]` Bij multi-dimensionale arrays in [4_ndimensionalArrays.md](4_ndimensionalArrays.md) staat *"de derde dimensie bestaat uit drie 2-dimensionale 2 bij 2 arrays"*. Voor een eerstejaars die nog moeite heeft met index 0 te starten, zijn 3D-arrays vermoedelijk een overload. Alles in één callout "voor wie ervan houdt" zou helpen. **(gedaan: het volledige 3D-verhaal, inclusief afbeelding en de indexering met drie getallen, zit nu in een dichtgeklapte callout "Voor wie ervan houdt: drie dimensies".)**
...
- `[>]` **``foreach``** wordt nergens in dit hoofdstuk gebruikt of vermeld, terwijl het exact het idiomatische antwoord is op "alle elementen overlopen". De ``for``-loop met ``i < arr.Length`` blijft het enige patroon. Dat is een gemiste kans, en ze zien ``foreach`` in oefeningen wel verschijnen. **(Bewuste keuze van Tim: ``foreach`` blijft voorbehouden voor H12 ([../11_arraysvanklassen/3_foreach.md](../11_arraysvanklassen/3_foreach.md)) en wordt hiervoor ook niet aangekondigd. TODO-comment vervangen door die beslissing; de vooruitwijzing naar "hoofdstuk 8" in H6 is rechtgezet naar hoofdstuk 12.)**
...
- `[>]` **``foreach``-iteratorvariabele is read-only**: typische valkuil. Mag in een callout. **(hoort bij H12, waar ``foreach`` uitgelegd wordt.)**
...
3. `[>]` Voeg in [1_ArraysBasics.md](1_ArraysBasics.md), na de "Lezen"-sectie met de manuele ``for``-loop, een korte ``foreach``-vergelijking toe in een callout. Liever nu dan dat het later "zomaar opduikt". **(Niet doen: ``foreach`` blijft bewust voor H12.)**
...
- **Wat doet dit?** — geef een fragment met `int[][] jagged` met variabele rij-lengtes uit de jagged-slotsectie van [4_ndimensionalArrays.md](4_ndimensionalArrays.md); laat student de output van een geneste loop voorspellen.
